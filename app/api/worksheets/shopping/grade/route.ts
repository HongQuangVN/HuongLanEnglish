...
const ANSWER_KEY: Record<number, string> = {
  ...
  2: "flea markets",
  3: "easily bored",
  4: "catches my attention",
  5: "avoid clothes shopping",
};
...
export async function POST(request: NextRequest) {
  ...
  const { error: insertError } = await supabase.from("submissions").insert({
    ...
    max_score: total,
    data: {
      fib: { "shopping": { items: detail, correct: correctCount, total } },
      translations: { "shopping": translations || [] },
      speaking: { "shopping": speaking || "" },
    },
  });
